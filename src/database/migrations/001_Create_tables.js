const up = async (db) => {
  ...
  await db.query(
    "CREATE TABLE `match`( id INT NOT NULL AUTO_INCREMENT, time TIMESTAMP DEFAULT CURRENT_TIMESTAMP, place VARCHAR(255), score INT,fk_team_home INT,fk_team_away INT, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (id) )"
  );

  await db.query(
    "ALTER TABLE `match` ADD FOREIGN KEY (fk_team_home) REFERENCES team(id), ADD FOREIGN KEY (fk_team_away) REFERENCES team(id)"
  );
